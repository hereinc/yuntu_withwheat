...
class YMMetric
  class << self
    def gobal_gmv time = Time.now
      ...
      result = OrderItem.paid.where('pay_time >= ? AND pay_time <= ?', time.beginning_of_year, time.end_of_year).sum('pay_price')
    ...
    def gmv_per_month
      month_gmv = OrderItem.group('YEAR(pay_time), MONTH(pay_time)').paid.select('pay_time, sum(pay_price) as day_gmv').map do |o|
    ...
    def gmv_per_day
      day_gmv = OrderItem.group('DATE(pay_time)').paid.select('pay_time, sum(pay_price) as day_gmv').map do |o|
    ...
    def purchase_frequency
      user_purchase_times = OrderItem.group('order_id, user_id').paid.size
    ...
    def total_repurchase_rate time_limit = Time.now
      user_purchase_times = OrderItem.group('order_id, user_id').paid.where('pay_time < ?', time_limit).size
